...
impl Settings {
    ...
    pub fn get_settings_view(&self) -> Element<Message> {
        ...
        root = root.push(widget::row::with_capacity(2)
            .push(widget::text::text(SettingName::EndOfFocusSound.to_string()))
            .push(dropdown));
        ...
        root = root.push(widget::row::with_capacity(1)
            .push(widget::text::text(SettingName::EndOfRelaxSound.to_string()))
            .push(dropdown));
        
        root.align_items(Alignment::Center)
            .into()
    }
